...
export const searchPostByTitle = catchAsyncError(async (req, res, next) => {
  ...
  if (!title) {
    return next(
      new ErrorHandler("Please provide a title to search", 400, false)
    );
  }
  const posts = await PostModel.find();
  const results = fuzzysort.go(title, posts, { keys: ["title", "tags"] });
  const searchResults = results.map((result) => result.obj);
  if (searchResults.length === 0) {
    const top5Post = await PostModel.find().limit(5);
